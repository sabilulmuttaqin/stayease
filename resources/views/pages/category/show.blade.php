...
@section('content')
    ...
    <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
        <div class="flex flex-col gap-[6px]">
            <h1 class="font-bold text-[32px] leading-[48px]">Kos in {{ $categoryBySlug->name }}</h1>
            <p class="text-ngekos-grey">Tersedia {{ $categoryBySlug->boarding_houses->count() }} Kos</p>
        </div>
        <button class="flex flex-col items-center text-center shrink-0 rounded-[22px] p-[10px_20px] gap-2 bg-white">
            ...
            <p class="font-bold text-sm">4.5/5</p>
        </button>
    </div>
    <section id="Result" class=" relative flex flex-col gap-4 px-5 mt-5 mb-9">
        ...
        @if ($boardingHouses->isEmpty())
            <p>Tidak ada kos di kategori ini.</p>
        @endif
