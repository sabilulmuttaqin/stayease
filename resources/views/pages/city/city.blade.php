...
@section('content')
    ...
    <div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
        ...
        <p class="font-semibold">Browse stayease</p>
        <div class="dummy-btn w-12"></div>
    </div>
    <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
        <div class="flex flex-col gap-[6px]">
            <h1 class="font-bold text-[32px] leading-[48px]">{{ $cityBySlug->name }} City</h1>
            <p class="text-ngekos-grey">Tersedia {{ $cityBySlug->boarding_houses->count() }} Kos</p>
        </div>
    ...
    <section id="Result" class=" relative flex flex-col gap-4 px-5 mt-5 mb-9">
        ...
        @foreach ($boardingHouses as $kos)
            <a href="{{ route('boarding-house.show', $kos->slug) }}" class="card">
                <div
                    ...
                    <div class="flex w-[120px] h-[183px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                        <img src="{{ Storage::url($kos->thumbnail) }}" class="w-full h-full object-cover" alt="icon">
                    </div>
                    <div class="flex flex-col gap-3 w-full">
                        <h3 class="font-semibold text-lg leading-[27px] line-clamp-2 min-h-[54px]">{{ $kos->name }}</h3>
                        <hr class="border-[#F1F2F6]">
                        <div class="flex items-center gap-[6px]">
                            <img src="{{ asset('assets/images/icons/location.svg') }}" class="w-5 h-5 flex shrink-0"
                                alt="icon">
                            <p class="text-sm text-ngekos-grey">{{ $kos->city->name }} City</p>
                        </div>
                        <div class="flex items-center gap-[6px]">
                        ...
                        <p class="font-semibold text-lg text-ngekos-orange">Rp
                            {{ number_format($kos->price, 0, ',', '.') }}<span
